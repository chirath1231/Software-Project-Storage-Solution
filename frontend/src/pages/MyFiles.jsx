...
export default function MyFiles() {
  ...
  const FileThumbnail = ({ file }) => {
      ...
      return (
        <div className="w-full h-full bg-gray-100 flex items-center justify-center overflow-hidden">
          <img src={file.url} alt={file.name} className="w-full h-full object-cover" loading="lazy" 
               onError={(e) => { e.target.style.display = "none"; e.target.parentNode.classList.add("fallback-thumb"); }} />
        </div>
      );
    }
    if (type === "video") {
      return (
        <div className="w-full h-full bg-black flex items-center justify-center overflow-hidden relative">
          <video src={file.url} muted preload="metadata" className="w-full h-full object-cover" onLoadedMetadata={(e) => { e.target.currentTime = 1; }} />
    ...
    if (type === "pdf") {
      return (
        <div className="w-full h-full bg-red-50 relative overflow-hidden">
          <iframe src={`${file.url}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`} title={file.name} className="w-full h-full border-0 pointer-events-none" style={{ transform: "scale(0.5)", transformOrigin: "top left", width: "200%", height: "200%" }} />
      ...
      return (
        <div className="w-full h-full bg-gradient-to-br from-pink-50 to-pink-100 flex flex-col items-center justify-center gap-2">
          ...
          <div className="flex items-end gap-0.5 h-6">
            {[4, 7, 5, 9, 6, 8, 4, 7, 5, 6, 8, 5].map((h, i) => <div key={i} className="w-1 bg-pink-300 rounded-full" style={{ height: `${h * 3}px`, opacity: 0.7 + (i % 3) * 0.1 }} />)}
          </div>
        </div>
      );
    }
    ...
    if (type === "text") {
      return (
        <div className="w-full h-full bg-white p-3 flex flex-col gap-1.5 overflow-hidden border-l-4 border-gray-200">
          {[80, 65, 90, 55, 75, 40, 85, 60].map((w, i) => <div key={i} className="h-1.5 bg-gray-200 rounded-sm" style={{ width: `${w}%` }} />)}
        </div>
      );
    }
    if (type === "word") {
      return (
        <div className="w-full h-full bg-white p-3 flex flex-col gap-1.5 overflow-hidden border-t-4 border-blue-600">
          ...
          {[90, 70, 85, 60, 75, 50, 80].map((w, i) => <div key={i} className="h-1.5 bg-gray-200 rounded-sm" style={{ width: `${w}%` }} />)}
        </div>
      );
    }
    if (type === "excel") {
      return (
        <div className="w-full h-full bg-white p-2 flex flex-col gap-0.5 overflow-hidden border-t-4 border-emerald-500">
          <div className="grid grid-cols-3 gap-0.5">
            {Array.from({ length: 18 }).map((_, i) => <div key={i} className={`h-3 rounded-sm ${i % 4 === 0 ? "bg-emerald-200" : "bg-gray-100"} border border-gray-200`} />)}
          </div>
        </div>
      );
    }
    ...
    return (
      <div className={`w-full h-full ${config.bg} flex flex-col items-center justify-center gap-2`}>
        {config.icon}
        <span className="text-[10px] font-bold tracking-widest" style={{ color: config.accent }}>{file.name.split(".").pop().toUpperCase()}</span>
      </div>
    );
  };
  ...
  // ==========================
  const ShareDropdown = ({ file }) => (
    <div className="absolute right-0 top-full mt-1 w-56 bg-white border border-gray-200 rounded-lg shadow-xl z-50 p-3 cursor-default" onClick={(e) => e.stopPropagation()}>
      <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Share Expiry Date:</label>
      <input
        type="datetime-local"
        className="w-full border border-gray-300 rounded-md px-2.5 py-1.5 mb-3 text-sm focus:ring-2 focus:ring-orange-500 outline-none"
        value={shareExpiry}
        onChange={(e) => setShareExpiry(e.target.value)}
      />
      <button
        onClick={async () => {
          if (!shareExpiry) { alert("Please select expiry date"); return; }
          try {
            const res = await api.post(`/api/files/${file.id}/share/`, { expiry_date: shareExpiry });
            navigator.clipboard.writeText(res.data.url);
            alert("Sharable URL copied! It will expire at " + new Date(res.data.expiry).toLocaleString());
            setOpenMenuId(null);
            setShareExpiry("");
            fetchGlobalNotifications();
          } catch (err) {
        ...
        className="block w-full text-sm font-medium bg-orange-500 hover:bg-orange-600 text-white px-3 py-2 rounded-md transition-colors"
      >
        Generate & Copy Link
      </button>
    </div>
  ...
  const FileCard = ({ file }) => {
    ...
    return (
      <div
        className="group bg-white rounded-xl border border-gray-200 hover:shadow-lg hover:border-gray-300 transition-all duration-200 cursor-pointer flex flex-col relative"
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        onClick={() => setPreviewFile(file)}
      >
        {/* Thumbnail area - Fixed overflow to allow dropdown menu to show */}
        ...
        <div className="px-3 py-2.5 flex items-center justify-between gap-2 border-t border-gray-100">
          ...
          {/* Transplanted Share Menu */}
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setOpenMenuId(isMenuOpen ? null : file.id)}
              className={`p-1.5 rounded-lg transition ${isMenuOpen ? 'bg-gray-100 text-gray-800' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-50'}`}
            >
              <span className="text-lg leading-none font-bold">⋮</span>
            </button>
            {isMenuOpen && <ShareDropdown file={file} />}
          </div>
        </div>
      </div>
    );
  ...
  const FileRow = ({ file }) => {
    ...
    return (
      ...
      >
        ...
        {/* Quick Actions & Transplanted Share Menu */}
        <div className={`flex items-center gap-1 transition-opacity ${isMenuOpen ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
          <button onClick={(e) => { e.stopPropagation(); window.open(file.url, "_blank"); }} className="p-1.5 rounded-lg hover:bg-gray-200 text-gray-500 transition">
            <ExternalLink size={14} />
          </button>
          <button onClick={(e) => { e.stopPropagation(); deleteFile(file.id); }} className="p-1.5 rounded-lg hover:bg-red-100 text-red-400 transition">
            <Trash2 size={14} />
          </button>
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setOpenMenuId(isMenuOpen ? null : file.id)}
              className={`p-1.5 rounded-lg transition ${isMenuOpen ? 'bg-gray-200 text-gray-800' : 'text-gray-500 hover:bg-gray-200'}`}
            >
              <span className="text-lg leading-none font-bold">⋮</span>
            </button>
            {isMenuOpen && <ShareDropdown file={file} />}
  ...
  const renderPreview = (file) => {
    ...
    if (type === "pdf") {
      return <iframe src={`${file.url}#toolbar=1&navpanes=1&scrollbar=1`} title={file.name} className="w-full rounded-lg border-0" style={{ height: "68vh" }} />;
    }
    if (type === "html") {
      return (
        <div className="flex flex-col gap-2">
          ...
          <iframe src={file.url} title={file.name} sandbox="allow-scripts allow-same-origin" className="w-full rounded-lg border border-gray-200" style={{ height: "60vh" }} />
        </div>
      );
    }
